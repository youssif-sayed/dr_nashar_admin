...
class _AppAddLectuerScreenState extends State<AppAddLectuerScreen> {
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: result2 == null
                        ...
                        onPressed: () async {
                          final pickedfile = await FilePicker.platform
                              .pickFiles(type: FileType.video,withData: true,allowMultiple: true);
                          setState(() {
                            result2 = pickedfile;
                            totalTasks = totalTasks + result2!.files.length;
                          });
                        },
                    ...
                    Container(
                      ...
                      child: result3 == null
                        ...
                        onPressed: () async {
                          ...
                          setState(() {
                            result3 = pickedfile;
                            totalTasks = totalTasks + result3!.files.length;
                          });
                        },
                    ...
                    ElevatedButton(
                      ...
                      onPressed: () async {
                        if (lectureName != ''&&price!=''&&result!=null&&result2!=null&&result3!=null) {
                          setState(() {
                            isLoading = true;
                          });
                ...
                child: Container(
                  ...
                  child: AnimatedSwitcher(
                    ...
                    child: finishedTasks == totalTasks
                      ...
                      value: finishedTasks / totalTasks,
  ...
  Future<void> pick_file(var result, var result2,var result3) async {
    if (result != null && result2 != null&&result3!=null) {
      ...
      await imgRef.putData(file!,SettableMetadata(contentType: 'image/${result.files.first.extension}'));
      final imgURL = await imgRef.getDownloadURL();

      addedMap.addAll({'image':imgURL});
      setState(() {
        finishedTasks++;
      });
      ...
      for (int i=0;i<result2.files.length;i++) {
        ...
        String videoName='app/lectures/${YearsData.selectedYear}/${YearsData.selectedSubject}/videos/${result2.files.elementAt(i).name}';
        Uint8List? videoFile=result2.files.elementAt(i).bytes;
        final videoRef = FirebaseStorage.instance.ref().child(videoName);
        await videoRef.putData(videoFile!,SettableMetadata(contentType: 'video/${result2.files.elementAt(i).extension}'));
        final videoURL = await videoRef.getDownloadURL();
        String name= result2.files.elementAt(i).name.substring(0, result2.files.elementAt(i).name.indexOf('.'));
        videoNames.add(name);
      ...
      addedMap.addAll({'videoName':videoNames});
      addedMap.addAll({'videos':videoURLs});
      List<String> docURLs=[];
      List<String> docNames=[];
      for (int i=0;i<result3?.files.length;i++) {
        print('uploading doc $i');
        String docName="app/lectures/${YearsData.selectedYear}/${YearsData.selectedSubject}/docs/${result3.files.elementAt(i).name}";
        Uint8List? docFile=result3.files.elementAt(i).bytes;
        final docRef=FirebaseStorage.instance.ref().child(docName);
        await docRef.putData(docFile!,SettableMetadata(contentType: 'application/${result3.files.elementAt(i).extension}'));
      ...
      addedMap.addAll({'docName':docNames});
      addedMap.addAll({'docs':docURLs});
      print(addedMap);
      FirebaseFirestore.instance.collection('${YearsData.selectedYear}-lectures').doc('${YearsData.selectedSubject}').collection('videos').doc('${lectureName}').set(addedMap);
      FirebaseFirestore.instance.collection('codes').doc('${addedMap['id']}').set({'AS-2023':{'UID':'','used':false,'expireDate':1}});
