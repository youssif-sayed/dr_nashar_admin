...
class FireWeb {
  ...
  static void update_favourite(int index, bool value) {
    FireWeb.yearsData?.values
        .elementAt(inhetanceSubjectNumbers[0])
        .values
        .elementAt(inhetanceSubjectNumbers[1])
        .values
        .elementAt(inhetanceSubjectNumbers[2])
        .values
        .elementAt(index)['favourite'] = value;
    final refrance = FireWeb.yearsData?.values
        .elementAt(inhetanceSubjectNumbers[0])
        .values
        .elementAt(inhetanceSubjectNumbers[1])
        .values
        .elementAt(inhetanceSubjectNumbers[2])
        .values
        .elementAt(index)['refrance'];
    final name = FireWeb.yearsData?.values
        .elementAt(inhetanceSubjectNumbers[0])
        .values
        .elementAt(inhetanceSubjectNumbers[1])
        .values
        ...
        .elementAt(index);
    if (value == true) {
      Map<String, dynamic> addToSlider = {
        '$name': {'refrance': '$refrance'}
      };
  ...
  static String get_path() {
    String pathname =
        '${FireWeb.yearsData?.keys.elementAt(inhetanceSubjectNumbers[0])}/'
        '${FireWeb.yearsData?.values.elementAt(inhetanceSubjectNumbers[0]).keys.elementAt(inhetanceSubjectNumbers[1])}/'
        '${FireWeb.yearsData?.values.elementAt(inhetanceSubjectNumbers[0]).values.elementAt(inhetanceSubjectNumbers[1]).keys.elementAt(inhetanceSubjectNumbers[2])}/';
    return pathname;
  }
  ...
  static Future<void> delete_lecture(int index) async {
    ...
    final desertRef = storageRef
        .child("${FireWeb.currentSubject.values.elementAt(index)['refrance']}");
    ...
    docRef.update({
      "${yearsData!.keys.elementAt(inhetanceSubjectNumbers[0])}.${yearsData?.values.elementAt(inhetanceSubjectNumbers[0]).keys.elementAt(inhetanceSubjectNumbers[1])}.${yearsData?.values.elementAt(inhetanceSubjectNumbers[0]).values.elementAt(inhetanceSubjectNumbers[1]).keys.elementAt(inhetanceSubjectNumbers[2])}.${currentSubject?.keys.elementAt(index)}":
          FieldValue.delete()
    });
  }
...
/////////////////////////////////////////////////////////////////////////////
  ...
  static void update_ME_favourite(int sana, int index, int mada, bool value) {
    FireWeb.MEData?.values
        .elementAt(sana)
        .values
        .elementAt(index)
        .values
        .elementAt(mada)['favourite'] = value;
    final refrance = FireWeb.MEData?.values
        .elementAt(sana)
        .values
        .elementAt(index)
        .values
        .elementAt(mada)['refrance'];
  ...
  static Future<void> delete_ME(int index, int sana) async {
    final storageRef = FirebaseStorage.instance.ref();
    final path1 =
        '${FireWeb.MEData?.values.elementAt(sana).values.elementAt(index).values.elementAt(0)['refrance']}';
    final path2 =
        '${FireWeb.MEData?.values.elementAt(sana).values.elementAt(index).values.elementAt(1)['refrance']}';
    ...
    docRef.update({
      "${MEData!.keys.elementAt(sana)}.${MEData!.values.elementAt(sana).keys.elementAt(index)}":
          FieldValue.delete()
    });
  }
...
/////////////////////////////////////////////////////////////////////////////
  ...
  static Future<void> delete_topStudent(int index) async {
    ...
    print(path);
    final desertRef = storageRef
        .child("${FireWeb.topStudentData?.values.elementAt(index)['photo']}");
...
/////////////////////////////////////////////////////////////////////////////
  ...
  static Future get_images() async {
    ...
    for (int i = 0; i < imagesData.values.length; i++) {
      ...
      final url = await storageRef
          .child("${imagesData.values.elementAt(i)}")
          .getDownloadURL();
  ...
  static Future<void> delete_image(int index) async {
    ...
    final desertRef =
        storageRef.child("${FireWeb.imagesData?.values.elementAt(index)}");
